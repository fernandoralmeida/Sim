...
namespace Sim.Modulos.ComercioAmbulante.ViewModel
{
    ...
    using Empreendedor.ViewModel.Reports;
    ...
    class vmReport : NotifyProperty
    {
        ...
        {
            get { return new mData().Tipos(@"SELECT * FROM SDT_Atendimento_Tipos WHERE (Ativo = True) ORDER BY Valor"); }
        }
            ...
            Task.Factory.StartNew(() => mdata.RAtendimentos(sqlcommand))
                .ContinueWith(task =>
                {
                    if (task.IsCompleted)
                    {
                        ...
                        Charts = RAtendimento(mReport, true);
                        BlackBox = Visibility.Collapsed;
                        StartProgress = false;
                    }
                    else
        ...
        private FlowDocument PreviewPrint(ObservableCollection<BarChartThemes> obj)
        {
            ...
            foreach (BarChartThemes charts in obj)
            {
        ...
        private FlowDocument PreviewInTable(mReportAtendimentos obj)
        {
            try
            {
                ...
                if (isOrigem)
                {
                    ...
                    rg3.Rows.Add(tbairro2);

                    tbairro2.Cells.Add(new TableCell(new Paragraph(new Bold(new Run("ORIGEM DO ATENDIMENTO"))) { Padding = new Thickness(5) }) { ColumnSpan = 2, BorderThickness = new Thickness(0.5), BorderBrush = Brushes.Black });
        ...
        private ObservableCollection<BarChart> RAtendimento(mReportAtendimentos reports, bool periodo)
        {
            ...
            if (periodo)
            {
                ...
                if (Atendimentos.Count > 0)
                {
                    ...
                    if (Origens.Count > 0)
                    {
                        ...
                        bci.Title = string.Format("Atendimentos / Ano");
                    ...
                    if (Tipos.Count > 0)
                    {
                        ...
                        bci.Title = string.Format("Atendimentos / Tipo");

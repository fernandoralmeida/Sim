...
namespace Sim.Modulos.Empreendedor.ViewModel.Viabilidade
{
    ...
    class vmMainContent : NotifyProperty
    {
        ...
        private void AsyncViabilidades(List<string> sqlcommand)
        {
            //BlackBox = Visibility.Visible;
            //StartProgress = true;

            Task.Factory.StartNew(() => mdata.ViabilidadesNow(sqlcommand))
                .ContinueWith(task =>
                {
                    if (task.IsCompleted)
                    {

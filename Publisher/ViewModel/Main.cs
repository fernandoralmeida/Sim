...
namespace Sim.Publish.ViewModel
{
    public class Main : INotifyPropertyChanged
    {
        ...
        {
            try { 
            if (!System.IO.Directory.Exists(Folders.Publish_Path))
                System.IO.Directory.CreateDirectory(Folders.Publish_Path);
        ...
        public List<string> Listar
        {
            get
            {
                return ViewModel.Files.Listar(ViewModel.Directories.Sim_Dir, @"*.*").ToList();
            }
        }
        ...
        public ICommand CreatePackageCommand
        {
            get
            {
                ...
                {
                    _createPackageCommand = new DelegateCommand(ExecuteCreatePackageCommand, null);
                }
        ...
        private void ExecuteCreatePackageCommand(object param)
        {
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                string _root = string.Format(@"{0}\{1}", Directories.Desktop, Folders.Temp);
                string _cache = string.Format(@"{0}\{1}", _root, "cache");
                string _xml = string.Format(@"{0}\{1}", _root, "xml");
                string _wallpaper = string.Format(@"{0}\{1}", _root, "Wallpapers");
                string _database = string.Format(@"{0}\{1}", _root, "DataBase");
                string _publish = string.Format(@"{0}\{1}", Directories.Desktop, Folders.Publish);
                string _local = string.Format(@"{0}\{1}", Directories.Desktop, Folders.LocalInstaller);
                ...
                if (Listar.Count > 0)
                {
                    foreach (string file in Listar)
                    {
                        ...
                        if (fi.Name == "Sim.App.Updater.exe")
                            System.IO.File.Copy(file, _root + @"\Sim.App.New.Updater.exe", true);
                        ...
                        if (fi.Extension == ".xml")
                        {
                            if (fi.Name == "sim_update.xml")
                                ...
                                System.IO.File.Copy(file, _xml + @"\" + fi.Name, true);
                        }
                        ...
                        if (fi.Extension == ".config")
                            if(fi.Name == "Sim.App.Desktop.exe.config")
                                System.IO.File.Copy(file, _root + @"\" + fi.Name, true);
                ...
                Files.Sim_Install_Name = @"\sim_full_install.gz";
                ...
                System.IO.File.Copy(string.Format(@"{0}\CSharp\Projetos\Sim\Updater\bin\Release\{1}", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Sim.App.Updater.exe"), _publish + @"\Sim.App.Installer.exe", true);
                System.IO.File.Copy(string.Format(@"{0}\CSharp\Projetos\Sim\Updater\bin\Release\{1}", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Sim.App.Updater.exe"), _local + @"\Sim.App.Installer.exe", true);
                System.IO.File.Copy(string.Format(@"{0}{1}", _publish, Files.Sim_Install_Name), _local + @"\sim_full_install.gz", true);

...
namespace Sim.Publish.ViewModel
{
    public class Version
    {
        ...
        private static void w_Xml(string valor, string chave)
        {
            ...
            docxml.Save(string.Format(@"{0}\CSharp\Projetos\Sim\Publisher\{1}.xml", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "sim_update"));
            docxml.Save(string.Format(@"{0}\{1}\{2}.xml", Directories.Desktop, Folders.Publish, "sim_update"));
        }

        public static List<string> Listar(string file, string elemento, string elementos, string item)
        {
            List<string> lista = new List<string>();
            var docxml = XDocument.Load(string.Format(@"{0}\CSharp\Projetos\Sim\Publisher\{1}.xml", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), file));
